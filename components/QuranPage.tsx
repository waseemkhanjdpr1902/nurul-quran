...
export const QuranPage = () => {
  ...
  const surahSlug = params?.surahSlug as string;
  ...
  useEffect(() => {
    if (selectedSurah && typeof window !== 'undefined') {
  ...
  const fetchSurahs = async () => {
    const cached = typeof window !== 'undefined' ? localStorage.getItem('quran-surahs') : null;
    if (cached) {
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          <div className="flex items-center justify-between mb-6">
            <button 
              ...
              onClick={() => { router.push('/quran'); setAutoPlay(false); setPlayingAyah(null); audioRef.current?.pause(); }}
              ...
              >
                ...
                <div className="mt-6 pt-6 border-t border-emerald-50 dark:border-emerald-900/20 flex justify-end">
                  <button
                    ...
                    onClick={() => router.push(`/tafseer/${selectedSurah.number}/${ayah.numberInSurah}`)}
