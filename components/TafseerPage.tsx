...
export const TafseerPage = () => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      setSelectedLanguage(localStorage.getItem('tafseerLanguage') || 'english');
      setFontSize(parseInt(localStorage.getItem('tafseerFontSize') || '16'));
      setArabicFontSize(parseInt(localStorage.getItem('arabicFontSize') || '36'));
    }
  }, []);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedSurah, setSelectedSurah] = useState<number>(surahNumber ? parseInt(surahNumber as string) : 1);
  const [selectedAyah, setSelectedAyah] = useState<number>(ayahNumber ? parseInt(ayahNumber as string) : 1);
  ...
  useEffect(() => {
    if (surahNumber && ayahNumber) {
      const s = parseInt(surahNumber as string);
      const a = parseInt(ayahNumber as string);
      setSelectedSurah(s);
      setSelectedAyah(a);
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (surah) {
      router.push(`/tafseer/${surah.number}/1`);
    }
  };

  const shareTafseer = () => {
    if (!tafseer || !surahNumber || !ayahNumber) return;
    const surah = surahs.find(s => s.number === parseInt(surahNumber as string));
  ...
  const navigateAyah = (direction: 'next' | 'prev') => {
    ...
    router.push(`/tafseer/${nextSurah}/${nextAyah}`);
  };
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto pb-24 md:pb-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <nav className="flex items-center gap-2 text-sm text-zinc-500">
          <Link id="tafseer-breadcrumb-home" href="/tafseer" className="hover:text-emerald-600 transition-colors">Tafseer</Link>
          {currentSurah && (
            <>
        ...
        {loading ? (
          ...
          >
            {surahs.map((surah) => (
              <button
                key={surah.number}
                onClick={() => router.push(`/tafseer/${surah.number}/1`)}
              ...
              <div className="relative z-10 text-center">
                ...
                <div className="flex items-center justify-center gap-4">
                  <button
                    ...
                    onClick={() => router.push(`/quran/${selectedSurah}-${currentSurah?.englishName.toLowerCase().replace(/\s+/g, '-')}`)}
                  ...
                  {selectedAyah > 1 ? (
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (prevSurah) router.push(`/tafseer/${selectedSurah - 1}/${prevSurah.numberOfAyahs}`);
                      }}
                      className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-xl transition-all"
                    >
                      <ArrowLeft size={18} />
                ...
                <div className="flex items-center gap-2">
                  {currentSurah && selectedAyah < currentSurah.numberOfAyahs ? (
                    ...
                    <button
                      ...
                      onClick={() => router.push(`/tafseer/${selectedSurah + 1}/1`)}
                      className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-xl transition-all"
                    >
                      Next Surah
                      <ArrowRight size={18} />
