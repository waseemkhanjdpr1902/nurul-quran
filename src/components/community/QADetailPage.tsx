...
export const QADetailPage: React.FC = () => {
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto pb-32 md:pb-8 animate-fade-up">
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-[32px] border border-emerald-100 dark:border-emerald-900/30 shadow-sm p-8 mb-8">
        <div className="flex items-center gap-3 mb-4">
          <span className="px-3 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-[10px] font-bold rounded-full uppercase tracking-wider">
            {question.category}
          </span>
          <span className="text-[10px] text-zinc-400 font-medium">{new Date(question.timestamp).toLocaleDateString()}</span>
        </div>
        <h1 className="text-2xl md:text-3xl font-bold text-emerald-900 dark:text-emerald-100 mb-4">{question.title}</h1>
        <p className="text-zinc-600 dark:text-zinc-400 leading-relaxed mb-8 whitespace-pre-wrap">{question.body}</p>
        ...
        <div className="flex items-center justify-between pt-6 border-t border-zinc-100 dark:border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-emerald-900 dark:text-emerald-100">{question.author_name}</p>
              <p className="text-[10px] text-zinc-400 uppercase tracking-widest">Question Author</p>
            </div>
          </div>
      ...
      <div className="mb-8">
        ...
        <div className="space-y-6">
          {answers.map((a) => (
            ...
            >
              <p className="text-zinc-600 dark:text-zinc-400 leading-relaxed mb-6 whitespace-pre-wrap">{a.body}</p>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-emerald-900 dark:text-emerald-100">{a.author_name}</p>
                    <p className="text-[10px] text-zinc-400">{new Date(a.timestamp).toLocaleDateString()}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <button
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-[32px] border border-emerald-100 dark:border-emerald-900/30 shadow-sm p-6">
        <h3 className="text-lg font-bold text-emerald-900 dark:text-emerald-100 mb-4">Your Answer</h3>
        <form onSubmit={handlePostAnswer} className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-4">
            <p className="text-[10px] text-zinc-400 italic">
              Posting as <span className="text-emerald-600 font-bold">{user?.displayName || 'Anonymous'}</span>
            </p>
            <button
              type="submit"
